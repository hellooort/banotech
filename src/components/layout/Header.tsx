          ...
          {openMega === 'contactUs' && (
            <div className="flex gap-4">
              {[
                ...
                { icon: Mail, label: t.mega.emailInquiry, desc: locale === 'ko' ? '문의 폼으로 이동합니다' : 'Open the inquiry form', href: '/support/inquiry' },
              ].map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
              ...
              <div key={item.key} className="border-b border-border last:border-0">
                ...
                {mobileSection === item.key && item.key === 'contactUs' && (
                  <div className="pb-2 pl-4">
                    ...
                    <Link href="/support/inquiry" onClick={() => setMobileOpen(false)} className="block py-2 text-sm text-muted hover:text-foreground">{t.mega.emailInquiry}</Link>
                  </div>
                )}
              </div>
            ))}
